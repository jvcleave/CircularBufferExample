...
void ofxThreadedImageLoader::loadFromDisk(ofImage* image, string filename) {
	lock();	
	//ofLogVerbose() << "filename: " << filename << endl;
	num_loading++;
...
void ofxThreadedImageLoader::urlResponse(ofHttpResponse & response) {
	if(response.status == 200) {
		...
		// log error.
		ofLogError() << "Could not image from url, response status: " << response.status;
		...
		if(it != images_async_loading.end()) {
			...
			ofLogWarning() <<  "Cannot find image in load-from-url queue";
		}
		unlock();
	}
}
